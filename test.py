...
def main():
    ...
    cfg.TEST.CHECKPOINTS = cfg.mld_path + '.ckpt'

    state_dict = torch.load(cfg.TEST.CHECKPOINTS, map_location="cpu")["state_dict"]
    from collections import OrderedDict
    new_dict = OrderedDict()
    for k, v in state_dict.items():
        if not k.startswith('reward_model.'):
            new_dict[k] = v
    state_dict = new_dict
    ...
    for i in range(replication_times):
        ...
        if "TM2TMetrics" in metrics_type and cfg.TEST.DO_MM_TEST and False:
            # mm metrics
    ...
    # cfg.output_dir, 'output.log'
    cfg.rcd_path = cfg.mld_path.split('checkpoints/E')[0] + 'results/' #  + cfg.mld_path.split('checkpoints/E')[1].split('/')[0] + '/metrics.json'
    os.makedirs(cfg.rcd_path, exist_ok=True)
    epoch = 'E' + cfg.mld_path.split('checkpoints/E')[-1].split('-')[0]
    with open(osp.join(cfg.rcd_path, f'{epoch}.json'), "w", encoding="utf-8") as f:
        json.dump(all_metrics_new, f, indent=4)
    import shutil
    shutil.copyfile(osp.join(cfg.output_dir, 'output.log'), osp.join(cfg.rcd_path, f'{epoch}.log'))
