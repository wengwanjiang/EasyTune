"""Fine-tuning script for MLD using chain-of-thought (EZtune) method.

This script fine-tunes a pre-trained MLD model using reward-guided optimization
with optional chain-of-thought (EZtune) training strategy. It supports multiple
fine-tuning methods: ReFL, DRaFT, DRTune, AlignProp, and EZtune.
"""
...
def save_checkpoint(model, policy_model, epoch, cfg, metrics, cur_rp1, cur_fid):
    """Save model checkpoint with metadata.

    Args:
        model: The MLD model.
        policy_model: Policy model (for EZtune method) or None.
...
def validation(target_model, val_dataloader, device, cfg, writer, global_step, ema=False):
    ...
    return max_val_rp1, min_fid, metrics
...
def main():
    ...
    ft_config["name"] += f"_{cfg.spm_path}"
    ...
    cfg.output_dir = f"./checkpoints/ft_mld/{cfg.spm_path}/{ft_config['name']}"
    ...
    # Create policy model for EZtune method
    policy_model = deepcopy(model)

    # Setup optimizer
    optimizer = torch.optim.AdamW(
        policy_model.denoiser.parameters(),
    ...
    policy_model.reward_model = reward_model
    ...
    policy_model.ft_config = ft_config
    policy_model.lambda_reward = ft_config["lambda_reward"]
    policy_model.reward_record = [[] for _ in range(50)]
    policy_model.trn_reward = []

    logger.info(f"FineTune Config: {ft_config}")

    # Initial validation
    _, _, metrics = validation(model, val_dataloader, device, cfg, writer, global_step)
    ...
    for epoch in range(10):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if ft_config["type"] == "EZtune":
                ...
                model = deepcopy(policy_model)

                if step % 50 == 1:
                    cur_rp1, cur_fid, metrics = validation(
                        policy_model, val_dataloader, device, cfg, writer, global_step
                    )
                    logger.info(f"Step {step}: R@1={cur_rp1:.4f}, FID={cur_fid:.4f}")
            else:
        ...
        cur_rp1, cur_fid, metrics = validation(
            policy_model if ft_config["type"] == "EZtune" else model,
            val_dataloader,
            device,
            cfg,
            writer,
        ...
        save_path = save_checkpoint(
            model, policy_model, epoch, cfg, metrics, cur_rp1, cur_fid
        )
        ...
        if cur_fid > 2 and cur_rp1 < 0.4:
            logger.info("Early stopping triggered: FID > 2 and R@1 < 0.4")
            break
