...
class MldVae(nn.Module):
    ...
    def decode(self, z: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        return feats
    def forward1(self, features: torch.Tensor, mask: torch.Tensor):
        """
        A tracing-safe forward pass for FLOPs computation.
        No distribution objects. No stochastic sampling.
        Only pure tensor operations.
        """
        # --- encode without creating Normal ---
        bs, nframes, nfeats = features.shape
        x = self.skel_embedding(features)
        x = x.permute(1, 0, 2)
        dist = torch.tile(self.global_motion_token[:, None, :], (1, bs, 1))
        ...
        mu = dist[0:self.latent_size, ...]
        # no logvar, no std, no Normal
        latent = mu.permute(1, 0, 2)

        # --- decode exactly same as original ---
        feats_rst = self.decode(latent, mask)
